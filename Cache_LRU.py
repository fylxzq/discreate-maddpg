...
import math
class CacheLRU:
    def __init__(self,contents,capacity,slot):
        self.base_num = 5
        # self.request_num = 5000
        ...
        for i in range(self.base_num):
            self.basestations.append(LRUBaseStations(self.contents,capacity))
    ...
    def step(self,fileName,step):
        ...
        while (slot_times < 50):
            for base in self.basestations:
                one_time_perstep = 0
                # base.cache_status.printFromHeadToTail()
                miss_list = []
                while(one_time_perstep < self.slot):
                    ...
                    if(request_id in base.cache_status_set):
                        ...
                        if (request_id in self.local_cached):  # 如果当前请求内容已在该时隙被处理过
                            base.cache_status.access(request_id)#访问一次内容id，作用是将该内容标记为最新访问
                            continue

                        if (one_request_time + one_time_perstep > self.slot):  # 大于一个时隙，退出循环
                            # 一个时隙结束，重置大部分辅助数据
                ...
                while len(miss_list) > 0:
                    ...
                    while (request_id not in base.cache_status_set and base.max_cache_capacity - base.cache_capacity < current_content_size):
                        remove_id = base.cache_status.pop()
                    ...
                    base.cache_status.put(request_id)
        ...
        trans_size = all_basestation_totoal_size / 50 / self.base_num
        print(hit_rate)
        print(trans_size)
...
class LRU:#双向链表加哈希表实现LRU
    ...
    def put(self,content_id):
        ...
        node.next = self.tail
        self.tail.pre = node
        self.map[content_id] = node

    def access(self,content_id):#移动最近访问的节点至链表末端
        node = self.map.get(content_id)
    ...
    def pop(self):
        node = self.head.next
        self.map.pop(node.value)
        self.head.next = node.next
        self.head.next.pre = self.head
        return node.value

    def printFromHeadToTail(self):#从头到尾打印元素
        node = self.head.next
...
def execute(content_num, zipf, capacity, n_agents, slot, name):
    ...
    redundancy_file_name = "datas/redundancy_datas/" + name + "_redundancy_zipf-" + str(zipf)[ 2 ] + "_size-" + str(
        capacity) + ".xls"
    for i in range(50):
        contents = Contents(content_num, zipf)
        lru = CacheLRU(contents,capacity,slot)
        hit_rate, trans_size = lru.step(redundancy_file_name,i)
...
if __name__ == '__main__':
    ...
    for zipf in [ 0.5,0.7,0.9 ]:
        for capacity in [ 300 ]:
            execute(content_num, zipf, capacity, n_agent, slot, "lru")
